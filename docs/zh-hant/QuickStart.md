...
關於 MCP 協定的詳細說明，請參閱 [MCP 官方文件](https://modelcontextprotocol.io/docs/getting-started/intro)。
...
關於方法參數、回傳值型別、同步/非同步的完整說明，請參閱 [Tools - 參數和回傳值說明](Tools.md#參數和回傳值說明)。

## 用戶端

### 準備 MCP 伺服器

1. 你可以準備 stdio 傳輸層的 MCP 伺服器
    - 如 `npx -y @modelcontextprotocol/server-everything stdio`（無需提前執行）
2. 也可以準備 http 傳輸層的 MCP 伺服器
    - 如 `npx -y @modelcontextprotocol/server-everything streamableHttp`（需提前執行）
...
你也可以使用本程式庫編寫的 MCP 伺服器，例如本文件前面章節範例程式碼建立的 MCP 伺服器。

### 初始化

一個典型的 MCP 用戶端程式碼如下所示：
...
> 單一用戶端只連接一個 MCP 伺服器。在智慧型代理人程式中，通常需要同時管理多個 MCP 伺服器，請參閱 [McpServerManager](McpServerManager.md)。
