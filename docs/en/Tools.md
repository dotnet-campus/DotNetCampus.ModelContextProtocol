...
> Both input and output JSON Schemas are generated at compile time, capturing information only available during compilation without runtime reflection. For generation rules and limitations, see [Compile-Time JSON Schema Generation](JsonSchemaGeneration.md).
