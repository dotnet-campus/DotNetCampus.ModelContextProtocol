...
MCP 协议规定一个客户端只能连接一个 MCP 服务器。但在智能体程序中，通常需要同时管理多个 MCP 服务器（内置工具、外部服务、岗位程序等），并在调用时按 {serverName}.{toolName} 格式定位目标工具。

下面是一个用于同时管理多个 MCP 服务器的管理器示例。

> 本示例遵循 [客户端"先创建后连接"原则](../knowledge/client-build-before-connect.md)：`McpClientBuilder.Build()` 只创建客户端对象，不触发 I/O。注册阶段同步完成，连接阶段并发执行。
...
- **注册与连接分离**：`AddHttp`/`AddStdio` 是同步方法，只保存配置（无 I/O），连接由 `ConnectAllAsync` 统一处理。这得益于 `McpClientBuilder.Build()` 不触发连接的设计。
- **并发连接**：`ConnectAllAsync` 通过 `Task.WhenAll` 并发连接所有服务器。每个服务器的超时独立，一个坏服务器不会阻塞其他服务器的连接。
- **UI 友好**：注册后可立即展示服务器列表（`ListServers()` 含未连接状态），无需等待连接完成。
- **错误容错**：单服务器连接失败不抛异常，而是记录 `state.IsConnected = false` 和 `state.Error`，不影响其他服务器。
- **O(1) 工具查找**：`_tools` 字典按暴露名称索引，`CallToolAsync` 无需遍历。
- **工具名冲突检测**：添加工具时检查暴露名称是否已被其他服务器占用。
